...
本项目关注的是多机械臂搬运任务的运筹学调度求解与二臂/三臂运行模式选择，不是机器人运动学、动力学建模或轨迹控制问题；机械臂在本文中主要作为可分配的作业资源参与调度优化。

因此，本文并不研究机械臂关节角、末端轨迹、动力学方程或控制器设计，而是将多机械臂搬运过程抽象为一个具有任务分配、执行排序、资源互斥、安全区占用和多目标优化特征的调度问题。本章主要回答三个问题：第一，实际搬运任务如何抽象为任务集合与执行模式；第二，时间、能耗和安全约束如何用数学符号表达；第三，如何建立同时考虑最大完工时间、总能耗和负载均衡的优化模型。
