...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{workspace_two_arm_example.png}
        \par\small (a) 两臂系统示意
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{workspace_three_arm_example.png}
        \par\small (b) 三臂系统示意
    \end{minipage}
...
本问题同时关注完成效率、运行能耗和机械臂利用均衡。由于这三个目标的重要性不同，本文采用序贯目标规划思想，而不是简单加权求和。令 $G_1,G_2,G_3$ 分别为最大完工时间、总能耗和负载均衡的理想目标值，引入正负偏差变量：
\[
    C_{\max}+d_1^- - d_1^+=G_1,
\]
\[
    E_{\mathrm{tot}}+d_2^- - d_2^+=G_2,
\]
\[
    B+d_3^- - d_3^+=G_3.
\]
...
\[
    ...
    \quad \text{在保持 }P_1\text{ 最优结果不变的条件下},
\]
\[
    P_3:\min \left(Md_3^+ + B\right)
    \quad \text{在保持前两级主要目标不变的条件下}.
\]
其中 $M$ 为足够大的优先级系数。该目标结构表示：首先保证整体任务尽快完成，其次在不牺牲时间目标的前提下降低能耗，最后在前两者基础上改善机械臂负载均衡。
