...
这种处理方式可以理解为先离散调度，后连续调速。前一层由整数规划、目标规划和隐枚举等方法确定调度结构，后一层在调度结构已知的条件下，通过非线性规划优化速度参数。\textbf{既避免了将离散变量和连续变量完全耦合所带来的求解困难，又能够进一步分析速度、完成时间和能耗之间的权衡关系。}
...
上述模型说明，本章非线性规划部分的核心不再是选择哪只机械臂执行任务，而是在调度方案已经确定之后，进一步选择合适的运行速度。后续两个问题将在这一共同模型基础上分别进行扩展。
...
0.618 法本身是一维搜索方法，不能直接用于二维变量 $(s_1,s_2)$。本文的处理方式是：先由内点法确定二维空间中的下降方向 $\boldsymbol{d}^{(k)}$，再只在该方向上搜索步长 $\alpha$。于是构造一维函数
\[
	\varphi(\alpha)
	=
	\Phi_{\mu}
...
由于障碍函数 $\Phi_{\mu}$ 在可行域内部保持凸性，而凸函数沿任意直线方向的限制函数仍为一维凸函数，因此 $\varphi(\alpha)$ 在可行步长区间内具有单峰性。0.618 法正适用于这种一维单峰函数的最小值搜索。因此，本文不是直接用 0.618 法求二维最优点，而是将二维优化转化为“方向确定 + 步长搜索”的形式，其中 0.618 法只用于确定每一步的最优步长。
...
\]
该目标函数表示：在所有 seed 中找到能耗最大的那一个，并使这个最坏能耗尽可能小。由于 $\max$ 函数直接出现在目标函数中不便于后续求解，本文引入辅助变量 $z$，将最坏情形能耗目标转化为上界约束：
\[
E^{(r)}(s_S,s_D)\leq z,
\qquad
\forall r\in\mathcal{R}.
...
需要注意的是，线性化只在当前点附近可靠。如果一步走得太远，一阶近似可能与原非线性函数偏差较大，导致候选解虽然满足线性化约束，但代回原模型后不可行。因此，本文在逐次线性规划中加入步长限制，也可以理解为信赖区域约束：
\[
...
在线性规划子问题求解后，程序不会直接接受候选解，而是将候选解代回原来的非线性模型中检查。若候选解满足所有 seed 的原始截止时间约束：
\[
C_{\max}^{(r)}(s_S,s_D)\leq D,
\qquad
\forall r\in\mathcal{R},
...
\forall r\in\mathcal{R},
\]
并且目标值 $z$ 相比当前解有所改善，则接受该候选解作为新的迭代点。若候选解代回原模型后不可行，或者目标改善不明显，则说明当前线性近似不够可靠，程序缩小信赖区域 $\Delta_k$，重新求解局部线性规划子问题。

由于本问题的线性规划子问题只有三个变量 $s_S,s_D,z$，规模较小，本文在实现中采用顶点枚举思想求解。线性规划的最优解若存在，通常可以在可行域的某个顶点取得。因此，程序将线性化后的约束整理为若干半空间约束，枚举可能的约束交点作为候选顶点，再筛选满足所有线性约束的可行点，并从中选取 $z$ 最小的点作为线性规划子问题的解。这样可以避免依赖外部大型优化求解器，也与本文主问题中自编算法的思路保持一致。
...
该方法的优点在于，它没有把多个 seed 的结果简单平均，而是把每个 seed 的时间约束和能耗约束都保留下来；同时又通过逐次线性化，把复杂的非线性鲁棒优化问题转化为一系列小规模线性规划问题。最终程序输出统一速度倍率 $s_S^{*},s_D^{*}$、最坏情形能耗 $z^{*}$、各 seed 的可行性检查结果以及 KKT 验证结果。由此可以进一步比较二臂和三臂模式在随机任务分布扰动下的稳健性差异。
...
需要注意的是，鲁棒问题采用逐次线性规划法求解，最终结果首先保证原非线性约束下的鲁棒可行性；KKT 检查则进一步用于判断该数值解是否能够作为严格的一阶局部最优点。若部分行未通过 KKT 检查，则应将其解释为鲁棒可行解或数值近似解，而不能直接称为严格最优解。
